...
def generateTSDataSet(dataframe,features,targets,n_points=100):
    ...
    ml_df = pd.DataFrame() #HC initiate new df
    for strain in strains:
        strain_series = {}
        #HC: strain-specific df with no values
        strain_df = dataframe.loc[(strain,slice(None)),:]
        
        #HC: NO FURTHER USE? display(strain_df.index) gives Int64Index([], dtype='int64', name='Time')
        ...
        for measurement in features + targets:
            ...
            #If feature write out points in DICTIONARY
            if measurement in features:
                strain_series[('feature',measurement)]=smoothed_measurement
            
            #if target calculate derivative + write out points and derivative  in DICTIONARY
